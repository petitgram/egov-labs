...
public class EgovMediaiOSAPIServiceImpl implements
	...
	protected static Log log = LogFactory.getLog(EgovMediaiOSAPIServiceImpl.class);
	...
	public boolean selectMediaFileInf(HttpServletResponse response, MediaiOSAPIFileVO vo) throws Exception {
		...
		try {
		    ...
		    */
			...
			log.debug("IGNORE1: " + e.getMessage());
			errorFlag = false;
		} finally {
			if (bStream != null) {
				try {
					...
					log.debug("IGNORE2: " + e.getMessage());
					errorFlag = false;
				}
			}
			if (in != null) {
				try {
					...
					log.debug("IGNORE3: " + e.getMessage());
					errorFlag = false;
				}
			}
			if (fis != null) {
				try {
					...
					log.debug("IGNORE4: " + e.getMessage());
					errorFlag = false;
				}
			}
		}
	...
	{				
		...
		try {
			...
			log.debug("IOException - Method : getAudioData() " + e.getMessage());
		}		
