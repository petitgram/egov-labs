...
public class SampleBeanPostProcessor implements BeanPostProcessor,
	...
	public Object postProcessBeforeInitialization(final Object bean,
		...
		AnnotationMethodMatcher annotationMethodMatcher = new AnnotationMethodMatcher(
				Debug.class);

		// advice(설정파일을 보면 customizableTraceInterceptor 임) + Pointcut 으로 구성된
		// Advisor
		DefaultPointcutAdvisor advisor = (DefaultPointcutAdvisor) context
				.getBean("debugAdvisor");
		...
		for (Method method : clazz.getDeclaredMethods()) {
			...
			if (annotationMethodMatcher.matches(method, clazz)) {
				...
				log.debug("maching method : " + clazz.getSimpleName() + "."
						+ method.getName());
				...
				if (advisor.getPointcut() == Pointcut.TRUE) {
					advisor.setPointcut(new ComposablePointcut(
									new RootClassFilter(clazz),
									annotationMethodMatcher));
					// 한번 대체된 Pointcut 에 대해서는 다음에 찾아진 Debug Annotation 이 달려있는
					// Bean 에 대한 Pointcut 을 union 시켜 나감.
				} else {
					ComposablePointcut pointcut = (ComposablePointcut) advisor
							.getPointcut();
					pointcut.union(new ComposablePointcut(new RootClassFilter(
							clazz), annotationMethodMatcher));
