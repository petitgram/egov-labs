...
<html lang="kr"> 
    <head>
        <title>Generate code - Menu</title> 
        ...
        function changeType(componentId) {
         ...
         function changeTab() {
             ...
             f.action="<c:url value="/generate/menu/dialog.do"/>";
             f.target="_self";
         ...
         function generate(status) {
             var f = document.forms[0];
             f.action="<c:url value="/generate/menu/dialogGen.do"/>";
		...
		<div data-role="page">
			...
			<div data-role="content" data-theme="d">
				...
				<div class="content-primary">
					<div class="egov-tit">
						<h1>Menu <span class="sub">- Dialog</span></h1>
					</div>					
					...
					<div class="gen-Group">
                        ...
                        <div class="gen-grid">
                            <ul class="c-attr" id="groupInfo">
                                ...
                                <li>
                                   <div class="gen-wid1"><strong>메뉴수</strong></div>
		                            <div class="gen-wid2">
                    ...
                    <div class="demo">
                        <div id="tabs">                     
                            <ul class="tabTitle" id="tabTitle">
                                ...
                                <li><a href="#tabs-${compIndex}" data-ajax="false">Menu ${compIndex}</a></li>
                                </c:forEach>
                            </ul>
                            <c:forEach var="compIndex" begin="1" end="${compCount}">
                            <div id="tabs-${compIndex}" class="gen-cont">
                                <div class="gen-grid">
                                    <ul class="c-attr">                                      
       									<li>
		                                   <div class="gen-wid1"><strong>메뉴명</strong></div>
				                            <div class="gen-wid2">
				                            	<c:set var="compTextName">compText_${compIndex}</c:set>
                                            	<input type="text" name="compText_${compIndex}" value="<c:out value='${xml.component[compTextName]}' default="menu ${compIndex}" />" data-role="none" class="gen-sele" /> 
				                            </div>                               
		                                </li>
		                                <li>
		                                   <div class="gen-wid1"><strong>효과</strong></div>
				                            <div class="gen-wid2">
				                            	<c:set var="compEffectName">compEffect_${compIndex}</c:set>
			                                    <select name="compEffect_${compIndex}" data-role="none" class="gen-sele">
			                                        <option value="attr:data-transition=slide" ${(xml.component[compEffectName] == 'attr:data-transition=slide') ? 'selected="selected"' : ''}>slide</option>
			                                        <option value="attr:data-transition=slideup" ${(xml.component[compEffectName] == 'attr:data-transition=slideup') ? 'selected="selected"' : ''}>slideup</option>
			                                        <option value="attr:data-transition=slidedown" ${(xml.component[compEffectName] == 'attr:data-transition=slidedown') ? 'selected="selected"' : ''}>slidedown</option>
			                                        <option value="attr:data-transition=pop" ${(xml.component[compEffectName] == 'attr:data-transition=pop') ? 'selected="selected"' : ''}>pop</option>
			                                        <option value="attr:data-transition=fade" ${(xml.component[compEffectName] == 'attr:data-transition=fade') ? 'selected="selected"' : ''}>fade</option>
			                                        <option value="attr:data-transition=flip" ${(xml.component[compEffectName] == 'attr:data-transition=flip') ? 'selected="selected"' : ''}>flip</option>
			                                        <option value="attr:data-transition=turn" ${(xml.component[compEffectName] == 'attr:data-transition=turn') ? 'selected="selected"' : ''}>turn</option>
			                                        <option value="attr:data-transition=flow" ${(xml.component[compEffectName] == 'attr:data-transition=flow') ? 'selected="selected"' : ''}>flow</option>
			                                        <option value="attr:data-transition=slidefade" ${(xml.component[compEffectName] == 'attr:data-transition=slidefade') ? 'selected="selected"' : ''}>slidefade</option>
			                                    </select> 
				                            </div>                               
		                                </li>
                                    </ul>
