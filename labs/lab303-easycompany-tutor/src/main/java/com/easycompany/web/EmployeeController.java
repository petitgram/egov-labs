...
public class EmployeeController {
    ...
    // @RequestParam 의  required 를 이용한다. example) @RequestParam(value = "var", required = true)
    @RequestMapping(value = "/employeeList.do")
    public String getEmpList(@RequestParam(value = "pageNo", required = false) String pageNo,
            SearchCriteria searchCriteria, ModelMap model, Map<String, Object> commandMap) throws Exception {
...
//        model.addAttribute("employee", employee);
    ...
    public String insertEmployee(@ModelAttribute Employee employee, BindingResult bindingResult, Model model) {
        ...
        // 원래 페이지(addemployee)를 다시 보여주고 에러 메세지를 뿌려준다.
...
//    }
    ...
    // TODO [Step 4-6-02] getEmployeeInfo 메소드에  updateEmployee.do 요청시 Employee 객체 를 가져오도록 ModelAttribute 를 설정한다.
    // 파라미터로 “employeeid” key 가 넘어온다. 이것으로 EmployeeService 의 getEmployeeInfoById 메소드를 호출하여  Employee를 구해 리턴한다.
